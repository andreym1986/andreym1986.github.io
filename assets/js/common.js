...
---------------------------------------------------*/
	...
	window.ScrollEffects = function() {
		...
		hasParallax.forEach(function(hParallax) {
			...
			var bgIframe = hParallax.querySelector("iframe");
			var parallax = gsap.fromTo( [bgImage, bgVideo, bgIframe], {y: '-20%', scale:1.15}, {y: '20%', scale:1, duration: 1, ease:Linear.easeNone});		
...
---------------------------------------------------*/
	...
	window.Shortcodes = function() {  	
		...
		$("#share").jsSocials({
            showLabel: false,
    		showCount: false,
    		shares: ["linkedin", "facebook", "twitter", "pinterest", "email"]
        });
...
---------------------------------------------------*/
	...
	// .imageLink
	$(".imageFancybox").mouseenter(function(e) {	
		gsap.to('#ball', {duration: 0.2, borderWidth: '2px', scale: 1, borderColor:'#fff',});
		gsap.to('#ball-loader', {duration: 0.2, borderWidth: '2px', top: 2, left: 2});
		$( "#ball" ).addClass("with-icon").append( '<i class="fa-solid fa-plus"></i>' );
	});
		
	$(".imageFancybox").mouseleave(function(e) {
		gsap.to('#ball', {duration: 0.2, borderWidth: '4px', scale:0.5, borderColor:'#999999',});
		gsap.to('#ball-loader', {duration: 0.2, borderWidth: '4px', top: 0, left: 0});
		$("#ball").removeClass("with-icon");
		$('#ball i').remove();
